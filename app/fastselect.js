...
(function (root, factory) {
    ...
    $.extend(Fastselect.prototype, {
        ...
        init: function (inputElement, options) {
            ...
            this.isSortable = !!this.$input.data('sortable');
        ...
        addChoiceItem: function (optionModel) {
            ...
            $(
                '<div data-text="'+ optionModel.text +'" data-id="'+ optionModel.id +'" class="'+ this.options.choiceItemClass +' '+ ((this.isSortable) ? this.options.choiceMoveClass : '') +'">'+
                $('<div>').html(optionModel.text).text() +
                '<button class="' + this.options.choiceRemoveClass + '" type="button">×</button>' +
                ((this.isSortable) ? '<button class="holder" type="button"></button>' : '') +
                '</div>'
        ...
        hide: function (inActive) {
            // console.log('me3' + inActive);
            if (inActive) {
        ...
        documentCancelEvents: function (setup) {
            ...
            if(this.$el.hasClass(this.options.activeClass) && (setup == 'off')){
                this.hide(1);
            }
        },

        setSelectedOption: function (option) {
            if (this.optionsCollection.isSelected(option.id)) {
                return;
            }
        ...
        removeSelectedOption: function (option, $choiceItem) {
            ...
            if ($choiceItem) {
                ...
                this.$el.find(selectorFromClass(this.options.choiceItemClass) + '[data-id="' + option.id + '"]').remove();
            }
    ...
    $.extend(OptionsCollection.prototype, {
        ...
        applySelectedValues: function (onDone) {
            ...
            this.each(function (option) {
                ...
                if (this.options.multipleValues && option.selected) {
                    ...
                    this.selectedValues[option.id] = true;

                } else {

                    option.selected = !!this.selectedValues[option.id];
        ...
        load: function (params, onDone) {
            ...
            return $.get(gee.apiUri + options.url, params, function (data) {
