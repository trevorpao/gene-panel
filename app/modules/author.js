...
(function (app, gee, $) {
    ...
    gee.hook('author/loadList', function (me) {
        app.arena.resetCurrent(me);
        app.arena.nextPage(me);
    });

    gee.hook('author/cancelForm', function (me) {
        me.closest('form').html('');
    });

    gee.hook('author/loadRow', function (me) {
    ...
    gee.hook('author/initForm', function (me) {
        let tmpl = me.data('tmpl');
        let box = $('#' + tmpl);
        let tmplName = app.module.name + tmpl;
    ...
    gee.hook('author/setRow', function (me) {
        ...
        if (!$.validatr.validateForm(form)) {
            ...
            app.author.set(form.serialize(), me);
            gee.hideModal();
        }
    });

    gee.hook('author/delRow', function (me) {
        if (confirm('確認刪除此記錄')) {
            app.author.del({ id: me.data('id') }, me);
        }
    });

    gee.hook('author/reactList', function (me) {
