;(function(app, gee, $){
    ...
    gee.hook('media/loadList', function(me) {
        app.arena.resetCurrent(me);
        app.arena.nextPage(me);
    });

    gee.hook('media/cancelForm', function (me) {
        me.closest('form').html('');
    });

    gee.hook('media/loadRow', function(me) {
    ...
    gee.hook('media/initForm', function (me) {
        let tmpl = me.data('tmpl');
        let box = $('#' + tmpl);
        let tmplName = app.module.name + tmpl;
        ...
        if (app.module.pid) {
            app.media.loadRow(app.module.pid, tmpl);
        } else {
            app.arena.renderBox(box, { item: { id: 0 } }, 1);
    ...
    gee.hook('media/setRow', function (me) {
        ...
        if (!$.validatr.validateForm(form)) {
            ...
            app.media.set(form.serialize(), me);
            gee.hideModal();
        }
    });

    gee.hook('media/delRow', function (me) {
        if (confirm('確認刪除此記錄')) {
            app.media.del({ id: me.data('id') }, me);
        }
    });

    gee.hook('media/reactList', function(me) {
